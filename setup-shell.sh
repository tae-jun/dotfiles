#!/usr/bin/env bash
# zsh + starship + atuin + zoxide + fzf + zsh plugins 셋업 (Ubuntu/Debian)
...
if ! fzf_ok; then
  ...
  ver=$(curl -fsSL https://api.github.com/repos/junegunn/fzf/releases/latest | grep -oP '"tag_name": "v?\K[^"]+')
  curl -fsSL "https://github.com/junegunn/fzf/releases/download/v${ver}/fzf-${ver}-linux_amd64.tar.gz" | tar xz -C "$LOCAL_BIN"
fi
